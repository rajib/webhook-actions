...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { build_info: @build.build_info, webhook_action: @build.webhook_action } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { build_info: @build.build_info, webhook_action: @build.webhook_action } }
